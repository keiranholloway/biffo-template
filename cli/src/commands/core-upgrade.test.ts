...
describe("buildPrBody — the ownership claim is scoped to the tool's own commit (#1999)", () => {
  function emptySummary(): Record<MergeStatus, number> {
    return {
      unchanged: 0,
      'take-theirs': 0,
  ...
  /**
   * The old body said, flatly about the whole PR: "Only template-owned paths
   * (see `core-manifest.json`) were touched — product, plugin, and infra files
   * were left untouched." True when written, routinely FALSE by merge time,
   * because an instance carrying a divergence declaration cannot go green
   * without a second, user-owned commit on the same branch (the pin-equality
   * test asserts both directions, so neither the bump nor the re-derivation can
   * land alone).
   *
   * Live cost on tabsii-com/tabsii-platform#1420: a prosecutor compared the
   * branch head to this claim, correctly found it false, and rejected the PR.
   * The split that followed deadlocked the upgrade against its own stacked PR —
   * three builder dispatches, a model escalation and a blocker filed at the
   * owner, with nothing wrong in the code at any point.
   *
   * Asserted as an ABSENCE of the unscoped sentence, not merely a presence of
   * the new one: a future edit that re-adds the flat claim alongside the scoped
   * wording would still be the defect, and a presence-only test would pass.
   */
  it('never states flatly that the PR touched only template-owned paths', () => {
    const body = buildPrBody('0.312.0', '0.312.5', planWith([ordinary]), noMigrations, 'dev')
    expect(body).not.toContain('Only template-owned paths')
    expect(body).not.toContain('were left untouched')
  })

  it('scopes the claim to the commit the tool itself wrote', () => {
    const body = buildPrBody('0.312.0', '0.312.5', planWith([ordinary]), noMigrations, 'dev')
    expect(body).toContain(
      'The commit generated by `biffo core upgrade` touches only template-owned paths',
    )
    expect(body).toContain('core-manifest.json')
  })

  it('warns that later commits on the branch are not covered by it', () => {
    const body = buildPrBody('0.312.0', '0.312.5', planWith([ordinary]), noMigrations, 'dev')
    expect(body).toContain('Any later commit on this branch')
    expect(body).toContain('not covered by that statement')
  })

  it('still reports which versions it moved between', () => {
    const body = buildPrBody('0.312.0', '0.312.5', planWith([ordinary]), noMigrations, 'dev')
    expect(body).toContain('**0.312.0**')
    expect(body).toContain('**0.312.5**')
    expect(body).toContain('biffo.core.json')
  })
})
