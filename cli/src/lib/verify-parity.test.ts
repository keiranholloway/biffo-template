...
const EXCLUDED: Record<string, { kind: 'network' | 'pr-time' | 'history' | 'slow'; why: string }> =
  {
    ...
    "uv run python scripts/error_branch_coverage.py --check --coverage coverage.json ${{ hashFiles('.github/workflows/rls-tests.yml') != '' && '--coverage rls-coverage.json' || '' }}":
      {
        kind: 'slow',
        why: 'measured 0.19s itself — it is excluded for what it DEPENDS on, not its own cost: it reads coverage.json, produced only by the `pytest --cov` above, which is measured at 51.2s and excluded as slow for that reason. Including this in the push gate would mean paying that run locally. The trailing expression (#637) does not change this: this repo has no rls-tests.yml, so it always resolves to nothing and this reads exactly the one file it always did',
      },
  }
