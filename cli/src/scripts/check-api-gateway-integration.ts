...
export async function runApiGatewayIntegrationCheck(): Promise<void> {
  ...
  try {
    ...
    if (message.includes('no .tf files found under')) {
      console.log(
        '· API Gateway integration-qualifier guard: not applicable — ' +
          `${message} This repo carries no Terraform at all, which is expected for a ` +
          'non-instance satellite. Skipping.',
      )
      return
    }
