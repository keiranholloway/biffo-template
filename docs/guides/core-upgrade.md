...
When present, the plan attaches it to the `body drift` entry (`MigrationCarryPlan.divergedBodies[].declared`, `cli/src/lib/core-migrations.ts`) and `printMigrationBodyDrift` (`cli/src/commands/core-upgrade.ts`) renders it: the classification and reason, and whichever of options 1/2 above the declaration rules out, struck through.

**The carry does not act on the declaration — that half is still reporting, and deliberately so.** Every already-carried file is left exactly alone, declared or not — a `replay-safe` declaration does not get auto-applied, and an `outcome-changing` one is not blocked any differently than plain undeclared drift is today. Acting on it is issue #751's decision, reasoned from a single real example (#670); #751 says explicitly that it wants two or three more real body changes to test the two-class model against before the carry starts *acting* on the declaration, rather than risking a wrong one silently telling an operator their database converged when it didn't. Until then, an undeclared `body drift` means exactly what it always has: judge it yourself, using the table above.

**Writing the marker is enforced, though.** A CI check in this repo — `biffo check migration-body-change`, wired into "Release Guards" — refuses a PR that changes an already-released migration's *hashed* body (the same normalisation `migrationBodyHash` above uses: no docstrings, no whole-line comments, no `revision`/`down_revision`) with no `# biffo:body-change:` marker at all. It does not judge whether the classification is *right* — a `replay-safe` label on an outcome-changing edit still passes, because the correctness of the label is exactly what #751 says needs more real examples before it can be enforced. It only makes the marker's *absence* loud, at the one point a template author can still cheaply add it. `9715636d` (#931) — a docstring-only addendum to migration `0010` — is the guard's own worked example of what must stay silent: no DDL changed, so `migrationBodyHash` reports no difference and the check never asks for a marker.
