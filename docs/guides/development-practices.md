...
| — | **RETRACTED — a finding filed from static bytes that a single page render disproved.** This row previously read *"The portal serves 404s as HTTP 200"*. It is **wrong in three separate ways**, and the compounding is the lesson. (1) I `curl`ed `/dashboard/new-idea-scout/`, grepped the HTML for `404`, found *"This page could not be found."* and called it the rendered outcome — but that string is Next.js's **not-found boundary definition serialised into the RSC flight payload**, a fallback the client router may never reach. I read a template as an outcome. (2) It is **not the portal**: `/dashboard/*` has no CloudFront cache behaviour, so it falls to the DEFAULT origin — `biffo-platform-app`, a different bucket — where the key genuinely exists (ETag `c8de446f…`, matching byte-for-byte what CloudFront served) beside `app-builder/`, `ideation-engine/`, `scaffolder/`. (3) The CDN 404→200 rewrite **was fixed long ago**: the live distribution returns 403→403 and 404→404, and the module carries a long comment about the old bug. **The disproof was in my first probe and I did not read it**: the genuinely-missing path returned `403` with `x-cache: Error from cloudfront`, while this one returned `200` with `Miss from cloudfront` — a successful fetch. Loading the page settles it in one action: it redirects to `/login?return_to=%2Fdashboard%2Fnew-idea-scout%2F` and renders the sign-in form. A real, working, auth-gated route. **The rule: a claim about what a page DOES requires rendering it; bytes only support claims about bytes.** Filed, not merely caught pre-filing — the sixth instrument error in one session and the only one that reached the page | **process** | biffo-template (this row) | diagnostic practice | **retracted** — no defect exists; the CDN, the route and the origin are all correct |
...
| — | **`enforce_admins: false` on ELEVEN of twelve estate repos makes every branch-protection rule advisory for the only human who merges, and no audit reports it.** Three repos carrying `strict: true` — `tabsii-intake`, `tabsii-geo`, `biffo-plugin-ideation` — showed merges the new stale-merge metric says were not up to date, which that gate makes impossible by construction. The explanation is that protection binds an admin **nowhere except `biffo-template`**, the single repo where `enforce_admins` is on. The daily estate audit checks that branches **are** protected (19 branches, all protected, `OK` every day) and never checks that the protection **binds anyone** — so a repo can show a full required-check list, pass the audit, and still be one where any of it can be walked past without a trace. Two consequences past the obvious: **H3’s comparator `tabsii-crm` is not gated the same way as its treatment repo**, a confound the experiment never recorded, and `tabsii-platform` — added to H3’s treatment arm the same day — is unbound too. Nothing was looking; this surfaced only because an unrelated metric disagreed with a setting | **fail-open** · visibility | estate-wide, 11 of 12 repos (via `staleMergeShare` disagreeing with `strict: true`) | biffo-template `scripts/protection-audit.sh` | **partly fixed** ([#1052](https://github.com/keiranholloway/biffo-template/issues/1052)) — the audit now reads `enforce_admins` in the same call and **fails** on protection that binds nobody, so the false green is gone. Measuring it corrected the figure: not 11 repos but **18 of 19 branches**, every `main` among them, and only `biffo-template@dev` bound. Two things are deliberately NOT fixed here, because they are decisions rather than defects: each repo must be bound or its bypass recorded, and `protectionParamsFor()` must take `enforce_admins` as a parameter the way `strict` already does — until it does, `biffo doctor --fix` writes the bypass back over any repo bound by hand. Diagnosing it found the cause the original row did not have: the value is written twice on purpose (scaffold-time resumability in `configureBranchProtection()`, and the backfill payload in `branch-protection-apply.ts:131`) while `modules/source-control/github/main.tf` sets it `true` — the CLI's temporary init-time value has been the estate's permanent one, and nothing compared them |
...
| [#1040](https://github.com/keiranholloway/biffo-template/issues/1040) | **FOUR wrong causal claims in one session, every one about an environment-dependent or intermittent failure, and every one falsifiable in about two minutes by running the thing in both states.** `core upgrade --apply` aborted at its push step. **(1)** *"The SSH remote authenticates as `keiran-tabsii`, which cannot write to `keiranholloway/*`."* Wrong: those remotes are **HTTPS**, and a credential helper was already configured. The `-c credential.helper=…` workaround appeared to fix it only because it was always run *after* a `pnpm install` — the install was doing the work and the override took the credit. **(2)** *"The tool pushes before installing dependencies, so the pre-push gate rejects it."* Filed as an issue, then contradicted an hour later: the tool completed all four steps in a fresh worktree with **no `node_modules`**, while a manual push in that same worktree still failed the gate. Both cannot be true. **(3)** Cause still not established; the issue is retitled to the symptom. **(4) A fourth, made an hour after writing this row up.** A local test failure was read as *"`dev` is red"* and filed against an innocent PR (#970); `dev`'s CI was green throughout, and the same fixture run against three refs gives identical output. The real cause was that `verify-no-ci.test.ts` asserts a property holding only where `gitleaks` is **absent** — green on the CI runner, red on any workstation that has it (#1045, #1048). **The tell was available every time and skipped every time: an intermittent or environment-dependent failure invites a story, and the cheap check — reproduce it deliberately in BOTH states before explaining it — is the one that gets skipped precisely because a plausible cause is already in hand.** Two costs beyond the wasted diagnosis: a wrong instruction propagated into a dozen agent prompts, where each one dutifully applied cargo; and the fourth claim led directly to pushing a docs PR with `--no-verify`, which is the counter-metric H4 pre-registered as refuting itself. **That this row had to be amended, by its own author, within an hour of being written is the strongest evidence in it** — knowing the pattern demonstrably does not prevent reproducing it, so the fix has to be a mechanical step (run it in both states) rather than an intention to be careful | **visibility** · process | my own diagnosis, across biffo-template and biffo-platform | diagnostic practice — retitle to the symptom when the cause is unproven; never pass an unverified cause into a sub-agent prompt as fact | **corrected** — [#1040](https://github.com/keiranholloway/biffo-template/issues/1040) now states the symptom and records all three explanations |
| — | **Two guards for one authorization concept drifted, and the test written to prevent exactly that drift could not see it.** tabsii's ADR-0010 makes `permission_code` authoritative and *superseding*, with a platform super-role short-circuit. `require_crud_permission` implemented that; `require_principal_crud_permission` was written to the **template's** semantics — the two axes ANDed, no admin bypass — so the same caller got different answers depending on which transport they arrived by. `test_crud_permission_code.py`'s own docstring asserts "both transports honour the same axis", but its single principal-transport test uses a rule with a code and no `required_role` — **a case the two semantics agree on**. It was green throughout. The drift surfaced only when core 0.211.0 distributed an upstream test (`test_crud_guard_agreement.py`, keiranholloway/biffo-template#1019) that varies the inputs case-for-case and went red on arrival. Reachability mattered and was checked rather than assumed: the both-axes disagreement is currently unbuildable, but the **platform-admin** one needed only the endpoint control plane doing the single thing it exists for — putting a `required_role` on a plugin table — on the mount (`/api/v1/internal/plugins/*`) that external plugin traffic actually reaches. **A test that asserts a property in prose and exercises only the agreeing cases is documentation, not a guard** | **drift** | tabsii-platform | tabsii-platform [#471](https://github.com/tabsii-com/tabsii-platform/pull/471) | **fixed** — both guard bodies now identical below the registry lookup; the upstream test kept, declared as divergence with tabsii's verdicts, plus super-role and indistinguishability classes upstream has no reason to carry |
| — | **An `aws --query` alias renamed a field to its own opposite, and the false reading was published to the user and written into durable memory before another session caught it.** `--query '{sandbox:ProductionAccessEnabled,...}'` returned `"sandbox": false`, which reads naturally as *"sandbox: false, so we are out of the sandbox"*. It means `ProductionAccessEnabled: false` — SES production access is **off**. The disconfirming value was in the same response the whole time (`SendQuota.Max24HourSend: 200.0`, the sandbox quota) and went unread because the aliased field had already answered the question. Cost is not the lookup — it is that a wrong fact propagated into a memory file and a status report, then took a second session to detect and a third correction to unwind. **Naming is part of the measurement**: an alias that inverts its source turns a correct API response into a wrong answer that no gate can catch | **visibility** | my own diagnosis, tabsii SES status | diagnostic practice — select fields under their own names (`{ProductionAccessEnabled:ProductionAccessEnabled}`), never a name that could be read as the negation | **corrected** — re-verified, memory and index rewritten with the cause |
...
> **Moved out of this file (#953).** The class tally is derived from
> `docs/practices/evidence.jsonl` and changes on every append, so committing it
> meant every two concurrent practices PRs conflicted by construction — always
> inside this block, which no human should be resolving.
>
> Read it in the dashboard, or regenerate it locally:
>
> ```bash
> node scripts/practices-evidence.mjs --markdown          # print it
> node scripts/practices-evidence.mjs --write             # -> docs/practices/tallies.generated.md (git-ignored)
> ```
>
> The ranking is the finding, so quote it from a fresh run rather than from
> memory — a number typed into prose here goes stale silently, which is the
> defect this page has already recorded against itself three times.
...
> **Moved out of this file (#953).** Same reason as the class tally above: this
> block is regenerated from the full dataset on every append, so it conflicted
> with every concurrent practices PR and the only correct resolution — discard
> both sides, re-run `--write` — was not discoverable from the conflict.
>
> ```bash
> node scripts/practices-evidence.mjs --write             # -> docs/practices/tallies.generated.md (git-ignored)
> ```
>
> The per-repo notes that used to sit beside the counts are hand-maintained and
> live in `REPO_NOTES` in `scripts/practices-evidence.mjs`; the generated file
> carries both.
...
A DDL-checksum revert in tabsii-platform (a pure comment restore, verified
correct independently via sha256 match against the failed deploy log) kept
failing its pre-push pg-test lane with a shifting set of errors — 32, then 4,
then 27 failing tests, no two runs the same, every individual failure passing
100% alone.

**Theory 1 — memory/swap, ~10m.** Swap showed `8.0Gi/8.0Gi` used after a long
session, which looked like the obvious cause. `vmstat`'s `si`/`so` columns
(actual page-in/page-out activity) were ~0 — nothing was actively thrashing,
so the full swap was stale residue from an earlier peak, not a live cause.
Closing ~86 stray Chrome tabs and re-running confirmed it: available memory
went from 10Gi to 13Gi and the failure recurred anyway.

**Theory 2 — inherent Postgres flakiness, ~5m.** The actual error,
`asyncpg.exceptions.InternalServerError: tuple concurrently updated` on a role
`GRANT`, looked like generic catalog contention that a single sequential
pytest run "shouldn't" be able to produce. It nearly got recorded as
undiagnosed lane flakiness.

**The real cause, found in under a minute:** `ps aux | grep pytest` showed a
second `verify.sh` running concurrently — first an uncommitted local worktree,
then, after that one finished and the failure recurred with yet another
different error set, a **completely different repo** (`biffo-template`, a
separate Claude Code session). `pg-test-db.sh` binds a fixed port (`55432`)
with zero isolation between concurrent runs, cluster-wide or cross-repo, and
Postgres roles are cluster-global — so two unrelated test suites hitting the
same container race on the same catalog rows. Filed as
[biffo-template#1114](https://github.com/keiranholloway/biffo-template/issues/1114)
(since fixed — see the corpus row above).

**The asymmetry worth naming:** the two wrong theories were each individually
plausible and each took real time to rule out (~15m combined), while the
actual cause — visible the whole time as an extra process in `ps aux` — took
under a minute once looked for. Both wrong theories shared a tell that went
unused: a **shifting set of failures across identical re-runs with no code
change** is the signature of external contention, not of anything in the code
or the environment's static resources. That signature should have pointed at
...
Two very different costs in one session, and the cheap-looking one was the
expensive one.

**The deliberate spend — ~35 min, correctly.** A core upgrade (0.211.0) arrived
carrying an upstream test that went red, and the question it posed was a genuine
RBAC decision: adopt the template's AND semantics for `permission_code`, or keep
tabsii's ADR-0010 supersede and declare the divergence. Answering it properly
meant establishing whether the choice was even observable — grepping every rule
source (`domains/tabsii/*_models.py`, both installed plugin manifests, every
registry JSON) to find that **nothing in the deployment sets `required_role` at
all**, then working out which of the two disagreements was actually reachable.
That is not overhead; it is the work, and it turned "a test is red" into a
reachable platform-admin defect on the mount external plugin traffic uses. **A
decision that takes half an hour to establish as unobservable is still a decision
made on evidence rather than taste.**

**The accidental spend — one aliased field, three corrections.** An
`aws sesv2 get-account --query '{sandbox:ProductionAccessEnabled,...}'` returned
`"sandbox": false` and was read as *"not in the sandbox"*. It means the opposite.
The wrong fact then travelled: into a status report to the user, into a durable
memory file, into a second session that re-checked and contradicted it, and back
into a third correction to unwind. Direct lookup cost: **under a minute.** Total
cost across sessions: **three separate write-ups and one cross-session
contradiction to resolve**, plus whatever the user's confidence in recorded facts
is worth.

The asymmetry is the finding. **A measurement error is not priced by how long the
measurement took — it is priced by how far the wrong answer travels before
someone contradicts it.** The half-hour of deliberate investigation produced one
durable correct answer. The sub-minute command produced a wrong one that needed
two sessions to retract, and it was *never* going to announce itself: the API
call succeeded, the JSON was valid, and the field was real. Only the name was a
lie.

**Cheap structural fix, and it is not "be careful":** never alias a `--query`
field to a name that could be read as its negation. `{sandbox:ProductionAccessEnabled}`
is a bug in the ruler. `{ProductionAccessEnabled:ProductionAccessEnabled}` cannot
be misread. The disconfirming evidence was in the same response either way
(`Max24HourSend: 200.0`), and went unread precisely because the aliased field had
already answered the question — which is what a misleading name does: it stops
you looking.

**Waiting, for completeness:** ~7 min CI on each of two PRs, ~10 min for the dev
deploy, ~2 min per full local Python suite (run twice). Unremarkable, and none of
it was retried — no rebase cycles this session, `strict: false` on tabsii-platform
(experiment H3) meant neither PR went BEHIND.
...
**~2h 45m of agent time took `biffo-template` from 38 open issues to 27.** The
split is the finding, because it is nothing like where the effort was expected to
go.

| Phase | Agent time | Outcome |
| --- | --- | --- |
| Verification (3 parallel agents, ~12 issues each) | **~15 min** | 6 issues closed with no code written |
| Building (5 parallel agents, partitioned by file) | ~1h 40m | 8 PRs merged, 5 more issues closed |
| Orchestration, measurement, corrections | ~50 min | 2 live defects found, 1 issue filed |

**Verification returned the most per minute by a wide margin.** Fifteen minutes
of read-only checking retired six issues — one already fixed with its test in
place, one asking for a guard that exists, one whose body was half disproved by
four merged PRs, one closable by a single CloudWatch query. Every one of those
would otherwise have been *planned, scoped and built*. The cheapest work in a
backlog is establishing which of it is real, and it is the step that gets
skipped because it produces no diff.

**Two things only a running system could answer.** #724 asked for "a measured
reduction in cold start"; the source showed the fix present, and the measurement
showed it had **worked** (p50 4035ms → 2212ms) — closable, but only by querying.
#924 went the other way: the source showed the mechanism landed, and CloudWatch
showed it **failing on every cold start** since. Two agents reading code reached
the wrong verdict on it. Neither issue could be settled from a checkout, and they
failed in opposite directions — so "read the code" is not conservative, it is
just unreliable in an unpredictable direction.

**Parallelism worked because the partition was by file, not by topic.** Five
build agents ran concurrently with zero merge conflicts between them. The two
issues sharing `deploy-app.yml` were given to *one* agent as sequential PRs;
splitting them across agents would have guaranteed a conflict. The shared
append-only files (`evidence.jsonl`, this document) were declared off-limits to
all five and written once, centrally, afterwards — which is the only reason this
...
**~6.5 hours of agent time, and the loop is the whole finding.** One deploy step
held five independent defects. They abort in execution order, so exactly **one is
visible per attempt** — every fix "revealed" a new bug that had been there the
whole time.

| # | defect | how it failed | fix |
| --- | --- | --- | --- |
| 1 | `../../scripts/resolve-core-version.sh` — wrong depth | exit 127 | #981 |
| 2 | script tested `biffo.core.json` against the caller cwd | exit 1 | #986 |
| 3 | bare `python -m compileall`, no `setup-python` | exit 127 | #988 |
| 4 | `fileb://../lambda.zip` read above where it was written | exit 252 | #989 |
| 5 | package over the inline cap; guard failed instead of degrading | exit 1 | #990 |

**The cost is the round trip, not the fixes.** Each is a one-to-three line change.
Discovering the next one costs a full **template PR → CI → `core-v*` tag → npm
publish → `core upgrade` → instance PR → CI → dispatched deploy** — call it
~40 minutes when nothing goes wrong, five times, serially, with no way to
shortcut it because layer N+1 is unobservable until layer N is deployed.

**Structural, not carelessness.** `biffo-template` **authors
`deploy-app.yml` and can never execute it** — the repo is non-deployable, it
publishes to npm. So every defect in that file is written where nothing can
detect it and surfaces only in whichever instance takes it next. Three of the
five are the same class (a relative path assuming the wrong working directory),
and the step's own comments already stated the correct depth three lines above a
call that contradicted it.

**What it cost downstream while unfixed:** deploys had been failing since before
PR #404. `tabsii-platform`'s `/health` reported **0.23.3** — 173 releases
behind — with every merged change unshipped, and no alert anywhere. Not an
outage: the pre-upgrade code served normally, which is exactly why it went
unnoticed.

**Partly fixed.** Three guards now fail in the repo that owns the file. The loop
itself is untouched — see *"Nothing exercises `deploy-app.yml` where it is
written"* below.
...
`biffo-workflow` §4.5 says to read the diff and *specifically ask whether the
justification was checked*. The commit explaining the CRUD-guard fix asserted
that the endpoint control plane "can add a `required_role` to a coded table by
PR at any time". Plausible, load-bearing for the whole argument, and **false**:
`registry-schema.json` is `additionalProperties: false` with no `permission_code`,
so a plugin manifest cannot declare one. Checking it surfaced the reachable path
the wrong claim had been standing in for — `platform_admin` is a *different*
Cognito group from `admin`, so a `required_role` on any plugin table 403s a
platform admin on the internal mount. **The code was correct throughout; the
reasoning was not, and no test could have told the difference.** The corrected
premise is strictly stronger than the one it replaced, which is the argument for
the step: it is not a formality that occasionally catches a typo, it is where an
argument gets audited.

**Reading the component that owns the decision, instead of the one that logs.**
Verifying a newly authored workflow, the orchestrator Lambda logged
`Received event` and returned in 612ms with no dispatch line — and the event
envelope's `tenant_id` differed from the definition's, which appeared to confirm
that nothing matched. Two independent signals, one wrong conclusion, one step
from a bug filed against working code. `GET /api/v1/orchestration/runs` reported
the run **succeeded** with an SES `message_id`. The plugin only forwards; Core
matches, against the service principal's tenant. **Corroboration between two weak
signals is not strength — both can be downstream of the same misunderstanding.**
The habit worth keeping: before concluding from what a component did *not* log,
establish whether that component was ever the one deciding.
...
the error-branch ratchet, the IAM-prefix guard. One session of actually using the feature on
dev produced **five defects**, and not one was reachable by any test in either repo. The
sharpest was a `CHECK` constraint that made its own feature impossible: `ck_lessons_has_content`
required a lesson to have content at INSERT, but authoring is create-then-attach, so *every*
lesson creation failed. Four merged milestones passed through it.

The structural reason is worth stating plainly, because it generalises past this feature:
**core's handler tests run on SQLite, the sibling's BFF tests run on fakes, and neither lane
carries the `tabsii` CHECK constraints, the RLS policies, or the bucket's CORS policy.** Every
one of the five lived in an interaction between two layers that no single layer's tests
instantiate — app and Postgres, app and S3 CORS, React and static-export navigation, client and
the API Gateway authorizer. A green suite is evidence about one layer at a time.

**The plan predicted this and the prediction was actionable.** `0013-lms-v1` named M5 as one of
two milestones that 'cannot be believed from CI' *before* it was built. That sentence is why the
click-through happened at all rather than being deferred as polish.
...
`infra/environments/dev/main.tf`, so pattern-matching would have put it there.
`main.tf` is **user-owned**, and `biffo core upgrade` fail-closes on paths it
does not own: the output would have reached newly-scaffolded instances and never
an existing one, leaving `ARTIFACTS_BUCKET_NAME` unset, the deploy silently
falling back to the inline path it was built to escape, and the template
believing the feature had shipped. One `core-manifest.json` read beforehand
turned that into a template-owned `artifacts.core.tf` carve-out. #243, #548 and
#568 are each the same mistake caught *afterwards*.

**Saying what was not verified, which produced a verification.** The report on
biffo-platform's deploy said plainly that its `/health` was unchecked because
its AWS account was unreachable from this workstation. That sentence is what
prompted "there are AWS creds for the biffo platform account" — and the check
then passed (`0.196.7`, matching the repo). Under-claiming cost nothing and
converted an assumption into evidence; the note it corrected had said the default
profile hit "an unrelated Biffo instance", which was wrong and had made a
reachable account look unreachable.


**Asking a sub-agent to say what it could NOT verify is what located the two
issues only a running system could grade.** Each verification agent was told
*"if you cannot verify without running something, say so rather than guessing"*.
Both flagged the same class of gap — post-deploy init duration, a live
click-through — and that list was precisely the set of issues worth checking
personally. Without the instruction the agents would have produced confident
verdicts from source on both, and one of those verdicts (#924 closable) was
wrong. **The honest non-answer is a routing signal**, not a shortfall: it says
which work cannot be delegated.

**Distrusting a sub-agent's stale-issue verdict, and confirming it at the
source.** An agent reported #946 STALE-WRONG — an issue a human filed
deliberately. Two greps confirmed it (`parseNameStatus` keeps every status;
`services/api/` is a `templateOwned` prefix, so added files are already
blocked). Cheap, and closing a deliberately-filed issue on an agent's say-so
would have been the wrong habit even when the agent is right. The general rule:
**every STALE verdict is an agent telling you to discard human work — confirm
it yourself.**

**An agent correcting my root cause, and the correction changing the plan.** I
attributed #924's constraint violation to a collision between the self-seed and
`006_*.sql`. The implementing agent found the SQL seeds `plugin_name =
idea-scout` while the violated key was `plugin_name = host` — different rows, so
the race is self-seed against self-seed. The fix was unaffected, but the
follow-up was: retiring the SQL is a **data** question, not a file deletion, and
with #912 having removed the fallback, deleting on my wrong reading would have
surfaced as a 502. Recorded because the correction arrived from a subordinate
agent and was accepted on its evidence, which is the direction that usually
fails.

**Read past the layer that is masking the truth (§5) — on my own verification,
not on the bug.** Confirming `biffo-platform`'s `/health` after a green
deploy, `https://dev.biffo.io/api/v1/health` returned a normal-looking HTML
page. Headers: `403`, `server: AmazonS3`, `x-cache: Error from cloudfront`
— the portal bucket answering an API path. Reading them instead of the body was
the difference between "the API is broken" and "this domain does not route the
API"; the API Gateway origin answered correctly. Same shape as #647, on a
different instance and path.

**Negative controls, on a classifier that would otherwise have flattered its
author.** The runner-kill classifier (#982/#985) was validated against **16 real
runs including 7 negative controls** — terraform apply/init, a destructive-plan
guard, three genuine step failures. It mattered because the classifier changes
H3's counter-metric, and H3 is an experiment I had extended hours earlier:
without controls, "failures went down" would have been indistinguishable from
"the author moved the goalposts". `biffo-template` — H3's treatment repo — came
out **unchanged at 2 failures / 54.7 red minutes**, which is the result that
makes the correction credible.

**Live-data validation caught two defects unit tests could not, in one day.**
`staleMergeShare` scored `tabsii-platform` at 44.7% while `strict` was
still on — impossible by construction — with three unit tests green. The
per-window projection then silently dropped `killedRunIds`, so every repo
reported `runnerKills: null`, again green. Both were found by running the
collector against the estate and asking whether the number was *possible* rather
than whether it was plausible.
...
| — | **The error-branch gate's verdict depends on which *test style* a module uses, not on whether its error branches are tested.** `scripts/error_branch_coverage.py` reads `coverage.json`, and coverage loses the trace on everything after the first `await` against a real in-memory aiosqlite engine. In `lms_media.py`, line 201 (`await _brand_tenant(...)`) is recorded as executed and line 205 — four lines later, same function — is not. So four `except` branches that WERE exercised (the tests asserted the 502s and the 400, and passed) were reported as *added with no test exercising them*, and the gate was not wrong to say so: nobody had evidence. The split is visible estate-wide — `test_fdd_admin.py` uses the real-engine pattern and its module sits at **65%**; `test_lead_stage.py` uses a fake session and sits at **100%** with five awaits of its own. Two consequences. The ratchet will keep pushing baseline growth for genuinely-tested code, which erodes the one thing it measures; and module coverage percentages are not comparable across modules that differ only in fixture style. Fixed locally by moving the error tests to a fake session (61% → 84%), but that is a workaround in one module, not a fix for the instrument | **visibility** · fail-open | tabsii-platform, building LMS M4 | not fixed — candidates: `concurrency`/`sigterm` coverage settings, or a documented rule that error branches are tested against a fake session | **unfiled** |
...
| — | **Nothing exercises `deploy-app.yml` where it is written, and five defects accumulated in one step because of it.** `biffo-template` owns the workflow and is non-deployable, so the file is authored in a repo that cannot run it and only ever executes in an instance. Defects therefore surface one per deploy attempt, each masking the next, at ~40 minutes per round trip (template PR → tag → npm → `core upgrade` → instance PR → deploy). Four of the five were **shell and path errors reachable with no AWS at all** — wrong `../` depth twice, a missing interpreter, a zip written and read at different paths. A smoke harness running the step's shell logic against a fixture tree, in the repo that owns it, would collapse that loop from five deploys to one local test. Guards now exist for three of the five shapes, which is strictly better and still reactive: each was written *after* its defect shipped | **visibility** · process | tabsii-platform, biffo-platform (authored in biffo-template) | biffo-template — a packaging smoke harness; not started | **open** |
| — | **A guard that excludes a category for one good reason silently excludes it for all reasons.** `workflow-relative-paths.test.ts` exempted build artifacts from its existence check because they legitimately do not exist at checkout — correct, and its own comment argues it well. Nothing said what *was* still checkable about them, so the exemption became total and bug 4 lived in it for as long as the file has existed. The recovered assertion (**agreement**: what a step's `zip` writes is what its `fileb://` must read) needs no filesystem and no knowledge of the artifact. Worth asking of every exclusion already in the estate's guards: *what remains true about the thing you just stopped checking?* | **fail-open** | biffo-platform deploy | biffo-template — pattern, not a single fix | **unfiled** |
| — | **`botocore` is bundled into every Lambda package and is already in the runtime.** Measuring tabsii-platform's package during the size incident: 184MB unzipped, of which **botocore ~24MB** is shipped despite AWS providing it. It is not the largest item (numpy + numpy.libs at 57MB, via shapely, is), but it is the only one that is pure waste rather than a real dependency. Not enough on its own to clear the inline cap (~68MB base64 after removal, still over), which is why it was not pursued under time pressure — but it is free size in every instance, forever, and nothing measures package composition at all | **process** | tabsii-platform | not fixed — exclude boto3/botocore from the package; revisit alongside the S3 artifacts bucket | **unfiled** |
| — | **A third-party API tightened a limit overnight and took the whole measurement programme dark in one step — and the guard that caught it named the wrong cause.** GitHub cut the GraphQL node budget server-side. `gh pr list --json commits` expands to `commits(first:100){nodes{authors(first:100)}}`, so against a 100-PR page it requests 100×100×100 = **1,000,000** possible nodes against a 500,000 ceiling. The estimate is computed from the **page size**, not from what a repo holds, so lowering `--limit` does not help — `1000`/`500`/`250`/`100` fail identically — and all **15 repos failed at once**. Nothing local had changed: `gh` was 2.96.0, a month old, and the same query minus `commits` still succeeded. The preflight guard ([#917](https://github.com/keiranholloway/biffo-template/pull/917)) worked exactly as designed and refused to write a hollow snapshot — but its diagnostic asserted _"the most likely cause is credentials: gh stores its token in the keyring"_ **unconditionally**, which was true of the 401 that motivated it and false here, sending the reader after a keyring that was working. So the day had **no data at all**, and the previous day's standup choice could not be closed: an outage in the measurement layer costs the *ritual* that depends on it, not just the numbers. Two lessons worth keeping separate — a metric pipeline resting on an external API's implicit limits will break on **someone else's** schedule, and **a guard that prints one hardcoded cause is not a diagnosis**: a confidently wrong one costs more than none, because the reader spends the time disproving it | **fail-open** · visibility | biffo-template daily practices cron (04:30 2026-08-02) | biffo-template `scripts/practices-metrics.mjs` | **fixed** ([#1086](https://github.com/keiranholloway/biffo-template/pull/1086)) — `commits` fetched per-PR (10,000 nodes, inside budget) only for upgrade PRs whose body lacks the marker; cost **measured rather than assumed** at 107 extra requests across 1242 PRs over 90 days (8.6%), after the first draft of the change guessed "a handful" and was caught reading the diff. `diagnoseTotalFetchFailure` now names a cause only when the error says so, and otherwise admits it does not know |
| — | **A REQUIRED CI check had no local counterpart at all, and the gate printed `verify passed` anyway.** tabsii-platform's `rls-test` lane — real Postgres, RLS policies — was **9 of 13** locally-catchable failing CI steps across the estate in 24h. Every one was a genuine assertion failure on a feature branch, plus one re-runnability failure from leaked state; all of them a local run catches. `scripts/verify.sh` contained **no reference to Postgres in any form**, so the gate reported green having never looked at a check that costs a ~7-minute round trip to discover. Locally the whole lane is ~2s to build the schema (alembic 0.74s + 85 DDL modules 1.3s) and ~28s for 310 tests, so the arithmetic is not close. The lesson is not "add this check": a gate claiming to mirror CI must be measured against CI's **actual required checks**, because a lane living in its own workflow file is invisible to a gate assembled from what the repo obviously has. Second-order, and the part that makes it a fail-open rather than a gap: the fix must distinguish _"this repo does not have the thing"_ from _"this repo **has** it and the gate is blind to it"_ — printing both as a quiet `n/a` is how a gap gets read as coverage | **fail-open** · visibility | estate-wide, concentrated in tabsii-platform (found ranking the 2026-08-02 standup) | biffo-template `scripts/verify.sh` | **partly fixed** ([#1089](https://github.com/keiranholloway/biffo-template/pull/1089)) — `pg-test` selects by the same `test_*_pg.py` convention the CI lane uses, has three distinct states, and carries CI's anti-fail-open assertions so a schema-less database FAILS rather than reporting `0 passed` as success. Local DB helper still owed by tabsii-platform#487; until then it warns rather than runs there |
| — | **An alerting path that existed, read as coverage, and had never once fired.** All three `notify-send` calls in `practices-daily.sh` were guarded on a non-empty `DBUS_SESSION_BUS_ADDRESS` — correct-looking, and under cron **always false**: the crontab exports `PATH` and nothing else, so a cron child has no session bus. The daily job is the only schedule that matters for these alerts, so the estate-audit notification had been a no-op since the day it was written. It surfaced only because the 2026-08-02 collector outage was found by a human happening to run the standup, **40 minutes after** a job that had already detected the problem and correctly refused to write bad data. The class: a guard written for the interactive case, applied to an automated one, where the condition it tests is **structurally unsatisfiable** — and nothing distinguishes "did not need to notify" from "could not notify". Corollary worth keeping separately: the reliable surface is the artefact people already look at, not a channel that has to be checked | **fail-open** · visibility | biffo-template `scripts/practices-daily.sh`, all three call sites | biffo-template `scripts/` | **fixed** ([#1090](https://github.com/keiranholloway/biffo-template/pull/1090)) — the bus address is reconstructed from systemd's `/run/user/<uid>/bus` rather than inherited, and an idempotent stale banner is stamped onto the bookmarked dashboard on abort without blanking the last good data |
| — | **A local gate's own cached state can manufacture failures that look exactly like your bug.** Building the local Postgres gate, a test database about an hour old — built before two unrelated PRs merged — produced **23 failures** in a module nothing in the working tree had touched. Rebuilt from the same tree it passed **336/336**, and passed 336 again immediately, so the lane was genuinely re-runnable and every failure was the stale schema. The instructive part is that the **first reading was wrong**: 23 failures on a second run against a reused database looks exactly like leaked test state, and that conclusion would have driven the wrong design — rebuild-every-time, or a hunt for a teardown bug that does not exist. Only rebuilding and running twice separated the two. Generalises past databases: any local gate holding cached state between runs — a container, a build cache, a fixture directory — must treat freshness as a first-class property, because a stale artefact does not fail loudly, it fails **as somebody else's code**. A gate producing failures that are not the developer's fault is worse than no gate; it is the fastest route to the bypass H4 pre-registered as refuting the programme | **visibility** · fail-open | tabsii-platform pg lane, while building the gate's database helper | biffo-template `scripts/pg-test-db.sh` | **fixed** ([#1093](https://github.com/keiranholloway/biffo-template/pull/1093)) — fingerprints migrations and DDL by CONTENT (mtime survives a branch switch; content does not), stores it inside the database so it cannot outlive a drop, rebuilds only on mismatch, and refuses to record a fingerprint against a database holding under half the declared policies |


**A scoreboard row was wrong within an hour of being written, by its author,
about the very failure it described.** The row on wrong causal claims said
"three"; a fourth happened while it was still in review, and the fourth was the
same shape as the first three. This page's implicit theory is that recording a
lesson prevents its recurrence. That is falsified here on the shortest possible
timescale, and it is worth taking seriously rather than treating as bad luck:
**writing a lesson down changes what you can recognise afterwards, not what you
do in the moment.**

What would actually have worked, in all four cases, was a mechanical step rather
than an intention: *before putting a cause into anything durable — an issue, a
sub-agent prompt, a commit message — reproduce it in both the failing and the
passing state.* That is now in `biffo-verify`, but note it is the third
behavioural rule added there this session, and the other two have not yet been
tested against a session that did not already know about them. A rule whose only
evidence comes from the session that wrote it is not yet evidence.

**Amending a scoreboard row silently forks it in the corpus.** Found while making
the amendment above: `--extract` pairs corpus rows to markdown rows by summary,
and #978 deliberately made unpaired rows **survive** rather than be deleted —
which is right for a removal and wrong for a rewording. Changing a row's opening
sentence therefore leaves the old row orphaned in `evidence.jsonl` and adds the
new one, so the corpus asserted both "three wrong claims" and "four wrong
claims" about the same incident, and every share derived from it counted the
incident twice. The duplicate-summary guard cannot see it, because the two
summaries genuinely differ. Removed by hand here; nothing prevents the next one.
An `--amend`-aware path, or a stable per-row id, would close it — the id is
probably the honest answer, since summaries are prose and prose gets edited.

**The repeat rate is the finding, and this page is not currently structured to
show it.** Four instances of the blind-zero defect landed in one day, in four
unrelated components, written by four different sessions — none of which knew
the other three existed. The scoreboard records failure *conditions* one at a
time, which is right for diagnosis and useless for spotting that the same shape
has now been paid for eight times. Nothing on this page answers *"which class
of defect is costing us most often?"* except by reading all of it, and the
class tally (visibility 32%, fail-open 23%) is far too coarse to point at
anything actionable.

Two things would help and neither exists: a **recurrence count** on a
scoreboard row, so the second and third instance of a known shape update a
number rather than adding a row nobody connects; and a rule that **the third
instance of any class stops being a bug and becomes a design task** — at that
point the individually-correct fixes are demonstrably not working. The
blind-zero class reached four before anyone named it, and it was named only
because they happened to land in one session.

Worth noting what did work: adding the check to the `biffo-verify` skill moves
it from "something someone noticed once" to "a question asked before the next
one is written". That is the only intervention in this whole file that acts
*before* the defect rather than after it.

**Nothing distinguishes "this mechanism shipped" from "this mechanism works",
and the backlog is where that gap hides.** #924 is the clean case: the fix
merged, distributed to the instance, and has failed on every cold start since —
invisible because a *second* mechanism writes equivalent rows, so the observable
state is correct while the thing under test has never once succeeded. An issue
tracking a mechanism is written before either claim is true and is never
revisited to distinguish them, so it ages into "presumably done". The estate has
no signal for *"this code path executed successfully in a deployed
environment"* — `/health` reports a version, which answers "did the deploy
land", a strictly weaker question. Until something answers the stronger one,
every mechanism-shaped issue needs a human to go and look, and that does not
scale past a backlog of this size.

**A 47-label taxonomy with 1 of 38 issues labelled is worse than no taxonomy.**
The labels existed and were plausible (`core`, `cli`, `migrations`, `portal`,
`ci-cd`); nothing applied them, so the backlog could only be read linearly,
so it was not read, so it grew. Labelling all 38 took minutes once the
verification evidence was in hand — but only *because* it was in hand.
Labelling from titles beforehand would have encoded the same wrong assumptions
the titles carried. The open question is what keeps it current: nothing in
`biffo-workflow` or the issue templates requires a label at filing time, so
this will decay back unless the filing path enforces it.
...
**Nothing distinguishes "I read the bytes" from "I saw it work", and only one
of those supports a claim about behaviour.** A finding was filed asserting the
portal served 404s as 200. Every input to it was real — a `curl`, a grep, a
string that genuinely appears in the HTML — and the conclusion was still wrong,
because the string was a serialised *not-found boundary definition* rather than
the rendered outcome. One page load disproved it: the route redirects to
`/login` and works. The header evidence was also present and unread in the very
first probe (`Miss from cloudfront` for a served object versus `Error from
cloudfront` for a genuinely missing one).

The gap is that a static fetch and a render look equally like "I checked". Six
instrument errors happened in the session that produced this page; five were
caught by arithmetic or an identity comparison before they mattered, and this
one reached the scoreboard — because nothing about grepping HTML announces that
...
| `groom-backlog` | **worked** | Ran end to end on tabsii-platform: verification batches caught issues already done or stale before any code got written, and the ranked remainder (issue #441's 49 unsatisfiable RLS grants, grouped A/B/C/D) turned into four real merged fixes plus this session's incidental discovery. Step 2's instruction to verify against live logs/deploys rather than source alone is what pushed a Chrome click-through for Group C before dropping 18 grants, rather than trusting the code-reading verdict. |
| `biffo-verify` | **worked — found the real cause of a flaky gate that two other theories had already half-explained** | Invoked while waiting on a blocked push (pg-test failing non-deterministically). Its "reproduce, don't theorise" discipline is what kept memory/swap and "inherent flakiness" from being written down as the answer — both were plausible, both had already cost real time, and neither was it. `ps aux` (not in the skill, but the same spirit as §4's "verify the artifact, not the theory") found a second concurrent `verify.sh`, first in another local worktree, then in a different repo entirely, both racing on the same hardcoded Postgres port. Filed as [biffo-template#1114](https://github.com/keiranholloway/biffo-template/issues/1114); see the cost note above. |
...
| `biffo-verify` | **partial — §6 has no rule for a name that inverts its source** | §6 covers a gate that passes when it cannot run, and a zero that means "could not see" rather than "nothing there". It does not cover the case that actually bit: a measurement that **ran correctly and reported truthfully**, under a label that meant the opposite (`--query '{sandbox:ProductionAccessEnabled}'` → `"sandbox": false`). Nothing about it looks like a failure — valid call, valid JSON, real field, and the disconfirming value (`Max24HourSend: 200.0`) sat unread in the same response because the aliased field had already answered. Suggested addition to §6: *never rename a field to something that could be read as its negation; select it under its own name.* This is a distinct failure mode from both cases §6 currently lists. |
| `biffo-verify` | **§8 is where the ad-hoc work nearly escaped** | Most of this session had no issue and, for the M8 half, no PR at all — the deliverable was an API call. The skill's insistence that §8 applies to ad-hoc work is the only reason the workflow-definition-as-data gap and the token recipe got recorded rather than evaporating with the session. |
| *not used, should have been* | `practices-standup` | The user had already recorded a "React-controlled form, no API alternative" row as a skills failure. `practices-standup` closes the loop on yesterday's recorded choice, and running it would have re-surfaced that row for validation before the session built on it. Instead the false half of the premise survived until it was tripped over by accident. Missed because the session opened with two concrete work items and went straight at them. |
...
| `groom-backlog` | **created this session — not yet independently exercised** | Written *from* this session rather than used to drive it, so it has no honest outcome yet; recorded now so the next invocation can be compared against it. The instructions it encodes that were learned the hard way: partition build agents **by file** (five ran concurrently with zero conflicts; the two issues sharing `deploy-app.yml` went to one agent as sequential PRs), declare the append-only corpus off-limits to every agent and write it once centrally (else #953 reproduces N times), and tell agents explicitly that a previous triage found issues stale — without that line, agents rationalise every open issue as real. |
...
| `practices-standup` | **worked — it rejected its own top-ranked item, which is the point** | Rank 1 (merge contention, 16.8h) survived interrogation; rank 2 (batched shared-sync, 2.9h/day) did **not**. Pricing the counter-cost first showed batching buys up to 24h of stale *gates* across 12 repos, that no debounce window matches the real arrival pattern (three changes spread over 12 hours), and that the dominant historical driver had already been fixed by rehearsal. Step 2's "interrogate the ranking rather than accepting it" is what stopped a measured-looking 2.9h/day from becoming a day's work. |
| `biffo-workflow` | **partial — no guidance for two agents on one file** | §1 says never modify a worktree you did not create, which is right and insufficient: PR #986 was open, red for 75 minutes, on a fix I had been asked to write. The skill has no answer for "another session already owns this change" — take it over, supersede it, or wait. I posted the diagnosis as a PR comment and asked the operator, which worked, but the escalation was invented rather than followed. A detached worktree off the remote branch (`git worktree add --detach origin/<branch>`) is the mechanism that let me push without touching their tree; it deserves to be in the skill. |
| `biffo-verify` | **worked — §5 on my own verification, not on a bug** | Confirming `/health` after a green deploy, the domain returned a plausible HTML page; the headers said `403` / `server: AmazonS3` / `x-cache: Error from cloudfront`. Without §5 that reads as "the API is broken" instead of "this domain does not route the API", and the deploy fix would have been doubted for the wrong reason. |
| `biffo-verify` | **partial — §8 reached only when the operator typed the command, for the fourth recorded time** | Sections 1–7 were applied continuously across ~6.5 hours; §8 was not started until `/biffo-verify` was invoked explicitly, and then *twice*, because the first invocation was overtaken by live unblocking work. This is now four consecutive recorded instances of the same failure, all in the same direction. The skill says §8 always applies; nothing in the flow *reaches* it, because the natural end of a task is a green merge and the skill is not invoked at merges. Worth treating as a defect in the trigger rather than in the operator. |
| `biffo-verify` | **worked — §7 was load-bearing twice** | Stating "biffo-platform's `/health` is unverified, I could not reach its account" rather than inferring it from a green deploy is what prompted the operator to say the credentials existed — which turned an assumption into a check, and the check passed (`0.196.7`, matching the repo). Under-claiming cost nothing and bought a real verification. |
...
| `biffo-sib-build` | **worked — Step 2.6's live click-through caught two real bugs neither test suite saw, at two separate milestones** | Closing out tabsii's 0011 plan (8 core milestones + 5 sibling-UI milestones), every core endpoint and every built sibling UI was loaded/exercised as a real user before being called done — not just deployed-and-green. This is what found [#431](https://github.com/tabsii-com/tabsii-platform/issues/431)'s two instances (scoreboard row above): both were merged, CI-green, unit-tested, and 500'd on the very first click. Three other milestones' click-throughs found nothing — which is the useful negative result, not a wasted check. Research done *before* writing code also paid off three separate times on one milestone (M6): reading `BrandWorkspace.tsx`'s existing sidebar-gating logic first showed a documented franchisee-role/app-shell scope item was already unnecessary, avoiding real extra build. |
| `biffo-verify` | **partial — §8 fired only once the operator explicitly invoked it, same seam as every prior recorded instance** | Sections 1–7's discipline (reproduce, verify the deployed artifact, fail-first on a fix) was applied continuously and unprompted across the whole session — it is what caught the tenant_id bug class above. §8's write-up did not start until the operator typed `/biffo-verify` partway through, mid-task, with more milestones still queued — consistent with every other recorded instance of this finding elsewhere on this page. Not re-argued as a new row; recorded here as one more data point for a trigger that keeps needing to be external. |
...
| `biffo-sib-imp` | **worked — the research phase changed the design before any code existed** | Planning LMS v1. Three parallel Explore agents against the real repos produced two findings that inverted parts of the approved ADR: `fn_authorized`'s ancestor widening is hard-coded to two permission codes (so the learner catalogue read had to become a policy disjunct, not a grant), and the generic list handler takes no query parameters (so every parent-scoped list needs a bespoke route). Neither is discoverable from the PRD or the ADR. The skill's insistence on researching the *target repo* rather than the spec is what produced them. |
