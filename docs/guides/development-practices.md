...
_Generated by `node scripts/practices-evidence.mjs --write`. **292** classified rows, ordered by count — the ranking is the finding, so it is not fixed to the list above._
...
_Generated by `node scripts/practices-evidence.mjs --write` from **292** rows in `docs/practices/evidence.jsonl`. Do not edit between the markers — `practices-evidence.test.mjs` fails when this block does not match the dataset._

| Repo | Fixes landing here | Notes |
| --- | --- | --- |
| **biffo-template** | 150 of 292 (51%) | Core API, CLI, CI, CDN module, skeletons, migrations, publish pipeline, repo settings, orchestration schema, write-back framework, the git-hook chain, the estate audits, the practices tooling itself |
| **tabsii-platform** | 26 of 292 (9%) | Divergence ratchet, repo settings, the RLS lane and its tests, raw-SQL portability, SES identity and bounce capture, the invite payload |
| **biffo-plugin-idea-scout** | 17 of 292 (6%) | Adapter seam, research search capability, its own stylesheet, release + publish workflows |
| **biffo-platform** | 14 of 292 (5%) | Instantiated infra — API Gateway routes, CDN, vendored-plugin resyncs, DDL seeds, log config |
| **tabsii-crm** | 14 of 292 (5%) | Its E2E harness, a repo setting that diverged, a timeline rendering a failed fetch as "nothing sent", the missing sibling proxy |
| **biffo-plugin-ideation** | 14 of 292 (5%) | A UI rendering a 500 as an empty state; its publish workflow; a dead manifest block; an analyst that never searched |
| **tabsii-intake** | 5 of 292 (2%) | CI generation, branch-protection contexts, the `python-jose` removal |
| **tabsii-marketplace** | 2 of 292 (1%) | `python-jose` removal; the credential-dependent build |
| **biffo-runners** | 1 of 292 (0%) | Runner fleet docs + fail-fast |
...
**Three fail-opens surfaced in one day, and the only one caught by a mechanism was
caught by a mechanism built the same day.** `ci_has()` returning true when the
