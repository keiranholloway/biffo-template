 ...
 * @param {string} block
 */
export function spliceTally(markdown, block) {
  const start = markdown.indexOf(TALLY_BEGIN)
  const end = markdown.indexOf(TALLY_END)
  if (start === -1 || end === -1 || end < start) {
    throw new Error(
      `tally markers not found in ${SOURCE} — expected ${TALLY_BEGIN} ... ${TALLY_END}. ` +
        'Refusing to append: a second copy of this table is the failure this exists to prevent.',
    )
  }
  return markdown.slice(0, start) + block + markdown.slice(end + TALLY_END.length)
}
...
function main() {
  ...
  if (has('--markdown') || has('--write')) {
    const block = tallyMarkdown(readEvidence(EVIDENCE))
    if (has('--write')) {
      writeFileSync(SOURCE, spliceTally(readFileSync(SOURCE, 'utf8'), block))
      process.stderr.write(`spliced fix-repo tally into ${SOURCE}\n`)
    } else {
