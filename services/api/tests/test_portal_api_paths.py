"""Every literal `/api/v1/...` path the frontend calls must exist on the Core API.
...
Every `.ts`/`.tsx` file under `apps/portal/src/**`, EXCLUDING `*.test.ts(x)`
...
are no concatenated or computed-prefix call sites in this repo today; this
...
no instance of that shape exists anywhere in this repo or in the two
...
FRONTEND_SRC = REPO_ROOT / "apps" / "portal" / "src"
...
#: it. Those paths are not the Core API's contract, so asserting them against its
...
#: Same-origin `/api/v1/...` prefixes that API Gateway routes to a DIFFERENT
#: integration, so they are not the Core API's contract even though the browser
#: sends them to the same host.
#:
#: The interpolated-prefix allowlist above cannot express this: these call sites
#: are ordinary relative literals with no computed base. They reach a different
#: Lambda because of a routing rule declared in Terraform, not because of
#: anything visible in the TypeScript.
#:
#: `/api/v1/plugins/` is the shared plugin host (ADR-0021). The routing is
#: explicit in this repo's own infrastructure, not inferred:
#:
#: - `infra/environments/dev/plugin-host.core.tf` declares
#:   `ANY /api/v1/plugins/{proxy+}`, `GET /api/v1/plugins/{name}/admin` and
#:   `GET /api/v1/plugins/{name}/admin/assets/{proxy+}` against the plugin-host
#:   integration.
#: - `modules/cloud/aws/cdn/main.tf` adds a matching `api/v1/plugins/*`
#:   CloudFront behaviour pointing at that gateway.
#:
#: Same safety property as the identifier allowlist: it is an ALLOWLIST, not a
#: wildcard. A prefix not named here still has to match a registered route or
#: FAIL, so a genuinely missing Core route cannot hide behind this. Entries are
#: counted and REPORTED as external rather than dropped — a guard that silently
#: discards inputs it cannot judge shrinks its own denominator and reports the
#: remainder as the whole, which is the failure shape this file exists to
#: prevent (AGENTS.md §2).
#:
#: Adding a prefix here is a claim about infrastructure. Verify it against the
#: Terraform before you make it — an unverified entry converts a real 404 into a
#: permanently green check.
EXTERNAL_PATH_PREFIXES: tuple[tuple[str, str], ...] = (("/api/v1/plugins/", "plugin-host"),)


def _external_path_prefix(raw: str) -> str | None:
    """The integration a literal same-origin path is routed to, when that
    prefix is declared as another integration's — otherwise None."""
    for prefix, integration in EXTERNAL_PATH_PREFIXES:
...
class ExtractedPath:
    ...
    #: (`EXTERNAL_BASE_IDENTIFIERS`), meaning it never reaches the Core API. Such a
...
def extract_api_paths(text: str, file: Path) -> list[ExtractedPath]:
    ...
    for _content, _start in _template_literals(text):
        ...
        if raw.startswith(API_PREFIX):
            ...
            # the Core API at all — counted and reported, never silently dropped.
            found.append(
                ExtractedPath(
                    file,
                    _line_number(text, _start),
...
def _frontend_source_files() -> list[Path]:
    """`.ts`/`.tsx` call sites under `apps/portal/src/**`, tests excluded.
...
def registered_route_paths() -> set[str]:
    """The Core API's own registered route templates, normalised for display.
    ...
    as biffo-template#1330 proposes: on the FastAPI version this repo
...
def test_every_frontend_api_v1_path_is_registered_on_the_bff() -> None:
    ...
    # A literal same-origin path under a prefix API Gateway routes to another
    # integration is out of scope for the same reason a `${CORE_API_URL}`-based
    # one is: it never reaches this app. Marked here rather than in the
    # extractor because it is a fact about routing, not about the syntax of the
    # call site — the TypeScript is an ordinary relative literal either way.
    extracted = [
        replace(p, external_base=_external_path_prefix(p.raw) or p.external_base) for p in extracted
    ]
    ...
    summary = (
        ...
        f"apps/portal/src/** ({_relative(FRONTEND_SRC)}); "
        f"{matched_count} matched a route the Core API registers, "
    ...
    for p in external:
        ...
        reason = (
            f"routed to the {p.external_base} integration"
            if _external_path_prefix(p.raw)
            else f"prefixed with ${{{p.external_base}}}"
        )
        lines.append(
            f"  EXTERNAL   {_relative(p.file)}:{p.line}  {p.raw!r} -> {reason}, declared external"
        )
    if unmatched or unresolved:
        ...
        lines.append(f"\nRoutes the Core API registers: {sorted(registered_route_paths())}")
...
class TestPathIsRegistered:
    """`path_is_registered` in isolation, against both this repo's real
...
class TestExternalPathPrefixes:
    """The same-origin integration allowlist must stay an allowlist.

    Its whole safety property is that it excuses only prefixes somebody has
    deliberately declared, after checking the Terraform. If it ever widened into
    "anything under /api/v1/ that looks routed elsewhere", a genuinely missing
    Core route would go permanently green — the failure this file exists to
    prevent, reintroduced inside the file itself.
    """

    def test_a_declared_prefix_is_recognised(self) -> None:
        assert _external_path_prefix("/api/v1/plugins/acme/admin") == "plugin-host"

    def test_an_undeclared_prefix_is_not_excused(self) -> None:
        assert _external_path_prefix("/api/v1/whoami") is None
        assert _external_path_prefix("/api/v1/finance/reports") is None

    def test_a_prefix_is_matched_at_the_start_only(self) -> None:
        # `/api/v1/admin/plugins/...` is a REAL Core route and must not be
        # excused just because "plugins/" appears somewhere in it.
        assert _external_path_prefix("/api/v1/admin/plugins/x/chat-agents") is None

    def test_every_declared_prefix_is_under_the_api_prefix(self) -> None:
        # A declaration outside /api/v1/ would silence call sites this guard
        # never looks at anyway, which would be a misleading entry rather than
        # a dangerous one — but it is still a sign the list is being misused.
        for prefix, integration in EXTERNAL_PATH_PREFIXES:
            assert prefix.startswith(API_PREFIX), prefix
            assert integration, f"{prefix} must name the integration serving it"
